...
class BeneficiaryIndividualReportController extends Controller
{
    ...
    {
        // 1. Obtener datos del método index (Asistencias, Rides, etc)
        $indexResponse = $this->index($request, $candidate);
        $baseData = $indexResponse->getData(true)['data'];

        // 2. Obtener datos del método scores
        $scoresResponse = $this->scores($request, $candidate);
        $baseData['scores'] = $scoresResponse->getData(true)['data'];

        // 3. Nombre del archivo
        $name = "Reporte_Individual_" . str_replace(' ', '_', $candidate->first_name) . "_" . now()->format('d-m-Y') . ".xlsx";

        return Excel::download(new \App\Exports\BeneficiaryIndividualExport($baseData), $name);
    }
}
