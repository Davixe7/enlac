...
class BeneficiaryIndividualReportController extends Controller
{
    ...
    public function export(Request $request, Candidate $candidate)
    {
        $candidate->load(['program', 'enlacResponsible']);
        $start = now()->startOfYear()->addMonths($request->month - 1);
        $end = $start->copy()->addMonths(5)->endOfMonth();
        ...
        $rides = DB::table('rides')
            ...
            ->when($monthsRange, function ($query) use ($monthsRange, $year) {
                foreach ($monthsRange as $index => $monthNum) {
                    $alias = "m" . ($index + 1);
                    $query->selectRaw(
                        "SUM( CASE WHEN MONTH(created_at) = ? AND YEAR(created_at) = ? THEN ((CASE WHEN departure_time IS NOT NULL THEN 1 ELSE 0 END) + (CASE WHEN return_time IS NOT NULL THEN 1 ELSE 0 END)) ELSE 0 END ) as $alias",
                        [$monthNum, $year]
                    );
                }
            })
        ...
        $areaAttendances = DB::table('attendances')
            ...
            ->when($monthsRange, function ($query) use ($monthsRange, $year) {
                foreach ($monthsRange as $index => $monthNum) {
                    $alias = "m" . ($index + 1);
                    $query->selectRaw(
                        "SUM( CASE WHEN MONTH(date) = ? AND YEAR(date) = ? THEN 1 ELSE 0 END ) as $alias",
                        [$monthNum, $year]
                    );
                }
            })
        ...
        $rows = [];
        $rows[] = ['Field' => 'candidate_full_name', 'Value' => $candidate->full_name];
        $rows[] = ['Field' => 'period', 'Value' => $periodLabel];
        $rows[] = ['Field' => 'daysCount', 'Value' => $daysCount];
        $rows[] = ['Field' => 'present', 'Value' => $attendances->present ?? 0];
        $rows[] = ['Field' => 'justified', 'Value' => $attendances->justified ?? 0];
        $rows[] = ['Field' => 'unjustified', 'Value' => $attendances->unjustified ?? 0];
        $rows[] = ['Field' => 'issues', 'Value' => $issues ?? 0];

        // rides per type
        foreach ($rides as $type => $item) {
            $rows[] = array_merge(['Field' => "ride_{$type}"], ['Value' => $item->total]);
        }

        // areaAttendances and appointments as separate rows
        foreach ($areaAttendances as $areaId => $item) {
            $rows[] = ['Field' => "area_{$areaId}", 'Value' => $item->total];
        }

        foreach ($appointments as $typeId => $item) {
            $rows[] = ['Field' => "appointment_{$typeId}", 'Value' => $item->total];
        }
        ...
        $export = new class($rows) implements FromArray, WithHeadings, ShouldAutoSize {
            ...
            public function headings(): array { return ['Field','Value']; }
        };

        $filename = 'beneficiary_individual_' . $candidate->id . '_' . $start->toDateString() . '_' . $end->toDateString() . '.xlsx';
