...
class GeneralReportController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        if (!$start || !$end) {
            return response()->json(['error' => 'Fechas requeridas'], 422);
        }

        $data = [];

        // Lógica idéntica a tu index
        $data['candidates']['evaluations'] = DB::table('evaluations')->whereBetween('created_at', [$start, $end])->distinct()->count('candidate_id');
        $data['candidates']['accepted']    = CandidateStatusLog::whereBetween('created_at', [$start, $end])->where('status', 'aceptado')->count();
        $data['candidates']['rejected']    = CandidateStatusLog::whereBetween('created_at', [$start, $end])->where('status', 'rechazado')->count();
        ...
        $promedioTotal = DB::table('attendances')
            ...
            ->value('promedio_final');
        $data['beneficiaries']['attendance'] = round($promedioTotal ?? 0, 2);

        $data['sponsors']['total'] = Sponsor::count();
        $data['sponsors']['beneficiaries'] = Candidate::whereHas('sponsors')->count();
        $data['sponsors']['enlac'] = 'N/A';

        $data['payments']['parent'] = Payment::where('payment_type', 'parent')->whereBetween('date', [$start, $end])->sum('amount');
        $data['payments']['sponsor'] = Payment::where('payment_type', 'sponsor')->whereBetween('date', [$start, $end])->sum('amount');

        $data['rides']['equine'] = DB::table('rides')->whereType('equine')->whereBetween('created_at', [$start, $end])
            ->selectRaw("SUM(IF(departure_time IS NOT NULL, 1, 0) + IF(return_time IS NOT NULL, 1, 0)) as total")->value('total') ?? 0;

        $data['rides']['rubio'] = DB::table('rides')->whereType('rubio')->whereBetween('created_at', [$start, $end])
            ->selectRaw("SUM(IF(departure_time IS NOT NULL, 1, 0) + IF(return_time IS NOT NULL, 1, 0)) as total")->value('total') ?? 0;

        $fileName = "Reporte_General_" . now()->format('d-m-Y') . ".xlsx";

        return Excel::download(new \App\Exports\GeneralReportExport($data, $start, $end), $fileName);
    }
}
