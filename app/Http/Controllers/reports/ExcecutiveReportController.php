...
class ExcecutiveReportController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        ->join('rides', 'rides.candidate_id', '=', 'candidates.id')
        ->selectRaw(
            "SUM(CASE WHEN type = 'equine' THEN (CASE WHEN departure_time IS NOT NULL THEN 1 ELSE 0 END) + (CASE WHEN return_time IS NOT NULL THEN 1 ELSE 0 END) ELSE 0 END) as equine, SUM(CASE WHEN type = 'rubio' THEN (CASE WHEN departure_time IS NOT NULL THEN 1 ELSE 0 END) + (CASE WHEN return_time IS NOT NULL THEN 1 ELSE 0 END) ELSE 0 END) as rubio"
        )
        ->whereYear('date', $year)
        ->groupBy('candidates.id');
        ...
        $attendances = Candidate::select('candidates.id')
            ->join('attendances', 'attendances.candidate_id', '=', 'candidates.id')
            ->selectRaw(
                "SUM(CASE WHEN attendances.status = 'present' THEN 1 ELSE 0 END) as present, SUM(CASE WHEN attendances.status = 'absent'  AND attendances.absence_justification_type IS NULL THEN 1 ELSE 0 END) as unjustified, SUM(CASE WHEN attendances.status = 'absent'  AND attendances.absence_justification_type IS NOT NULL THEN 1 ELSE 0 END) as justified"
            )
            ->where('attendances.type', 'daily')
            ->whereYear('attendances.date', $year)
            ->groupBy('candidates.id');
        ...
        $scores = Candidate::select('candidates.id', 'candidates.first_name', 'candidates.middle_name', 'candidates.last_name')
            ...
            ->whereYear('ads.date', $year)
            ->selectRaw(
                "CONCAT_WS(' ', candidates.first_name, candidates.middle_name, candidates.last_name) as full_name, DATE_FORMAT(ads.date, '%m') as month, SUM(CASE WHEN ads.color = 'positive' THEN 1 ELSE 0 END) as positive, SUM(CASE WHEN ads.color = 'warning' THEN 1 ELSE 0 END) as warning, SUM(CASE WHEN ads.color = 'negative' THEN 1 ELSE 0 END) as negative"
            )
            ->groupBy('candidates.id', 'candidates.first_name', 'full_name', 'month');
        ...
        $export = new class($rows) implements FromArray, WithHeadings, ShouldAutoSize {
            ...
            public function headings(): array { return ['full_name','present','justified','unjustified','issues','rubio','equine']; }
        };

        $filename = 'executive_report_' . $year . '_' . time() . '.xlsx';
